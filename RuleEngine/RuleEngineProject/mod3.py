...
# Function to evaluate the combined AST against provided data
def evaluate_rule(ast, data):
    if ast.type == "operand":
        # Extract the left side and operator from the operand
        left, operator, right = re.split(r' ?(>|<|=|!=|>=|<=) ?', ast.value.strip())
        
        # Fetch the corresponding value from the data dictionary
        left_value = data.get(left.strip(), None)
        right_value = right.strip().strip("'")  # Remove quotes if present
        
        # Convert right_value to appropriate type
        if right_value.isdigit():
            right_value = int(right_value)  # Convert to int for numerical comparison
        else:
            # If it's not a number, keep it as string
            right_value = right_value.strip('"')

        # Ensure left_value is an int if it's a number in data
    ...
    return False  # Default return value for unhandled cases
...
data =  {"age": 28, "department": "Marketing", "salary": 25000, "experience": 6}

# Evaluate the combined AST against the data
result = evaluate_rule(combined_ast, data)
print(f"\nEvaluation Result: {result}")  # Output should be True or False based on the rules
